...
class rAudioBuffer(ctypes.Structure):
    ...
    _fields_ = [("buffer", ctypes.c_byte * 392)]


class rAudioProcessor(ctypes.Structure):
    """dummy structure"""

    _fields_ = [("buffer", ctypes.c_byte * 24)]
...
class Vector2(ctypes.Structure):
    ...
    _fields_ = [("x", ctypes.c_float), ("y", ctypes.c_float)]  # Vector x component  # Vector y component
...
class Transform(ctypes.Structure):
    ...
    _fields_ = [("translation", Vector3), ("rotation", Quaternion), ("scale", Vector3)]  # Translation  # Rotation  # Scale


class BoneInfo(ctypes.Structure):
    """Bone, skeletal animation bone"""

    _fields_ = [("name", ctypes.c_char * 32), ("parent", ctypes.c_int)]  # Bone name  # Bone parent
...
class Ray(ctypes.Structure):
    ...
    _fields_ = [("position", Vector3), ("direction", Vector3)]  # Ray position (origin)  # Ray direction
...
class BoundingBox(ctypes.Structure):
    ...
    _fields_ = [("min", Vector3), ("max", Vector3)]  # Minimum vertex box-corner  # Maximum vertex box-corner
...
class float16(ctypes.Structure):
    ...
    _fields_ = [("v", ctypes.c_float * 16)]


class GuiStyleProp(ctypes.Structure):
    """Style property"""

    _fields_ = [("controlId", ctypes.c_ushort), ("propertyId", ctypes.c_ushort), ("propertyValue", ctypes.c_uint)]
