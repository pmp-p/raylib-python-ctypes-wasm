...
if os.name == "nt":
    ...
    build = sysconfig.get_config_var("HOST_GNU_TYPE")
    dllname = f"libraylib.{build}.so"
    ...
    for function_to_wrap in functions_to_wrap:
        if function_to_wrap["name"] not in wrapped_functions_names:
            ...
            try:
                ...
                f = wrap_function(
                    function_to_wrap_ctype["name"], function_to_wrap_ctype["parametersTypes"], function_to_wrap_ctype["returnType"]
                )
                add_function_to_module(current_module, name_of_function, f)
            except AttributeError as e:
                warnings.warn(f"cannot import {name_of_function} : {e}")
